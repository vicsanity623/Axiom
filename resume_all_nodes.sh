...
# Resume All Nodes Script
# Opens three terminal windows and resumes all nodes: bootstrap, peer1, peer2

echo "=== RESUMING ALL 3 NODES ==="
echo "This script will open three terminal windows:"
...
create_node_command() {
    ...
    echo "conda activate Axiom10 && cd '$SCRIPT_DIR' && echo '=== RESUMING $node_type NODE (Port $port) ===' && python3 -m axiom_server.node --host 0.0.0.0 --p2p-port $port --api-port $api_port"
}

# Create commands for all three nodes
...
echo "Opening terminal for Bootstrap node..."
